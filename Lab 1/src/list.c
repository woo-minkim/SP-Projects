...
/* Swaps two list elements in parameters. */
...
void list_shuffle(struct list *list)
{
  ...
  for (e = list_begin(list); e != list_end(list); e = list_next(e))
  {
    struct list_elem *random_elem = list_begin(list);
    int count = 0;
    int random_count = rand() % list_size(list);
    while (count < random_count)
    {
      random_elem = list_next(random_elem);
      count++;
    }

    list_swap(e, random_elem);
  }
}
